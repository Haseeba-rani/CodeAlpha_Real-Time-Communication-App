      ...
      try {
        ...
        recognition.onend = () => {
          ...
          if (!cancelled && meetingIsLiveRef.current && !muted) {
            window.setTimeout(() => {
              if (!cancelled && meetingIsLiveRef.current && !muted) {
                try { recognitionRef.current = recognition; recognition.start(); } catch { setSpeechError('Live transcription paused. You can continue with manual transcript lines.'); }
              }
            }, 250);
          }
        };
